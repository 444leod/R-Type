        ...
        void _send(const asio::ip::udp::endpoint &dest, const UDPPacket &packet)
        {
            ...
            if (compressed.empty()) {
                ...
                this->_socket.async_send_to(
                    ...
                    std::bind(&NetworkAgent::_handleSend, this,
            ...
            finalPacket.insert(finalPacket.end(),
                reinterpret_cast<const std::byte*>(&magicNumber),
                reinterpret_cast<const std::byte*>(&magicNumber) + sizeof(magicNumber));

            finalPacket.insert(finalPacket.end(),
                reinterpret_cast<const std::byte*>(&compressedSize),
                reinterpret_cast<const std::byte*>(&compressedSize) + sizeof(compressedSize));
            finalPacket.insert(finalPacket.end(), compressed.begin(), compressed.end());

            this->_socket.async_send_to(
                asio::buffer(finalPacket), dest,
                std::bind(&NetworkAgent::_handleSend, this,
                    asio::placeholders::error,
                    asio::placeholders::bytes_transferred));
        }
