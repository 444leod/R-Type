...
{
public:
    explicit CollisionSystem(ecs::Registry &registry) : AUpdateSystem(registry, "CollisionSystem") {}
...
private:
    ...
    if (transform1.rotation == 0 && transform2.rotation == 0) {
        return (transform1.x < transform2.x + rect2.width &&
                transform1.x + rect1.width > transform2.x &&
                transform1.y < transform2.y + rect2.height &&
                transform1.y + rect1.height > transform2.y);
    }

    const float angle1 = transform1.rotation * (M_PI / 180.0f);
    const float angle2 = transform2.rotation * (M_PI / 180.0f);

    const float cosAngle1 = std::cos(angle1);
    const float sinAngle1 = std::sin(angle1);
    const float cosAngle2 = std::cos(angle2);
    const float sinAngle2 = std::sin(angle2);

    auto rotatePoint = [](float x, float y, float cosAngle, float sinAngle) {
        return std::make_pair(x * cosAngle - y * sinAngle, x * sinAngle + y * cosAngle);
    };
    ...
    static bool checkCircleRectangleCollision(const Circle& circle, const Transform& circleTransform, const Rectangle& rect, const Transform& rectTransform)
    {
        ...
        const float circleDistanceX = std::abs(rotatedCircleX);
        const float circleDistanceY = std::abs(rotatedCircleY);
        ...
        const float cornerDistance_sq = std::pow(circleDistanceX - rect.width / 2, 2) +
                                  std::pow(circleDistanceY - rect.height / 2, 2);
