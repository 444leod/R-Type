...
{
public:
    explicit DrawButtonsSystem(ecs::Registry &registry, engine::ResourcesManager& resourcesManager) : ARenderSystem(registry, "DrawButtonsSystem"), _textures(resourcesManager.textures()), _fonts(resourcesManager.fonts()) {}
    ...
    void execute(sf::RenderWindow &window) override
    {
        ...
        sf::Sprite sprite;
        sf::Text label;
        _registry.view<Button, Transform>().each([&] (const Entity& entity, Button& button, const Transform& transform) {
            ...
            } else
            {
                ...
                {
                    sp.textureRect = sf::IntRect(0, 0, tex.getSize().x, tex.getSize().y);
                }

                sprite.setTexture(tex);
                sprite.setTextureRect(sp.textureRect.value());
