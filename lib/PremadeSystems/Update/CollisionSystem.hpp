...
private:
    ...
    auto rotatePoint = [](const double& x, const double& y, const double& cosAngle, const double& sinAngle) {
    ...
    static bool checkCircleRectangleCollision(const shape::Circle& circle, const Transform& circleTransform, const shape::Rectangle& rect, const Transform& rectTransform)
    {
        ...
        const double cornerDistance_sq = std::pow(circleDistanceX - rect.width / 2, 2) +
