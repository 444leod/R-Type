...
/**
 * @brief Class representation of a game server, receives packet and can
 * broadcast
 */
class Server : public NetworkAgent {
  public:
     ...
     */
    Server(asio::io_context& ctx) : NetworkAgent(ctx, 25565) { std::cout << "Server started, listening on port: " << this->_port << "..." << std::endl; }
    ~Server() = default;
  ...
  private:
    ...
    std::map<PACKET_TYPE, std::function<void(const asio::ip::udp::endpoint& client, UDPPacket& packet)>> _packet_handlers = {{PACKET_TYPE::CONNECT,
                                                                                                                                 [&](const asio::ip::udp::endpoint& client, UDPPacket& packet) {
